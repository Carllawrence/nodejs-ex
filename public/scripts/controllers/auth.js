    ...
    function ($scope, $http, $uibModal, $rootScope, $timeout, $stateParams, $location, $state, AuthService, Upload) {
      ...
      if ($rootScope.currentUser) {
        ...
        $scope.saveUser = function (data) {
          ...
          $http.put('/api/appusers/' + $rootScope.currentUser._id, data.address);
        };
      ...
      $scope.forgot = function () {
        ...
        $http.post('/api/forgot', $scope.formData).then(function (response) {
          if (response.data.success) {
      ...
      $scope.reset = function (formData) {
        $scope.resetMsg = '';
        $http.post('api/reset/', {password:formData.password2, uuid:$stateParams.uuid}).then(function (response) {
      ...
      $scope.changepass = function (data) {
        ...
        $http.post('api/changepassword/', { password: data.password, username: $rootScope.currentUser.username })
