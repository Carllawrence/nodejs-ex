...
angular.module('starsApp')
  .controller('RegistrationsController', function ($rootScope,$window, $filter, Upload, Payslip, $scope, $state, $http, AuthService, $timeout,  User, filterFilter, $location) {
   ...
   $scope.$watch(function () {
	...
	$http.put('/api/appusers/'+ $rootScope.currentUser._id, {status:2, schoolYear:$rootScope.settings[0].schoolYear}).then(function(response){
...
$http.get('/api/payslips/'+ $rootScope.currentUser.regNumber).then(function(result){
