      ...
      $http.post('/api/login',
        ...
        .then(function (response) {
          ...
          if(response.status === 200 && response.data.status){
            ...
            $http.get("/api/profile").then(function(user) { 
...
console.log($rootScope.currentUser)
  ...
  $http.post('/api/register', {username: username, password: password, picUrl:picUrl, firstname: firstname, lastname: lastname, status:0})
    // handle success
    .then(function (response) {
        if(response.status === 200 && response.data.status){
